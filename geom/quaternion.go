...
func (this *Quaternion) Normalize() {
	mag := 1.0 / float32(math.Sqrt(float64(this.X*this.X+this.Y*this.Y+this.Z*this.Z+this.W*this.W)))
	this.X *= mag
	this.Y *= mag
	this.Z *= mag
	this.W *= mag
...
func (this *Quaternion) FromAxisAngle(axis *Vector4, angle float32) {
	sin_a := float32(math.Sin(float64(angle * .5)))
	cos_a := float32(math.Cos(float64(angle * .5)))
	this.X = axis.X * sin_a
	this.Y = axis.Y * sin_a
	this.Z = axis.Z * sin_a
	this.W = cos_a
...
func (this *Quaternion) FromEulerAngles(ea *Vector4) {
	...
	cosX, sinX := float32(math.Cos(float64(halfX))), float32(math.Sin(float64(halfX)))
	cosY, sinY := float32(math.Cos(float64(halfY))), float32(math.Sin(float64(halfY)))
	cosZ, sinZ := float32(math.Cos(float64(halfZ))), float32(math.Sin(float64(halfZ)))
...
func (this *Quaternion) Slerp(qa *Quaternion, qb *Quaternion, t float32) {
	...
	if dot < .95 {
		...
		s := float32(1.0 / math.Sin(float64(angle)))
		s1 := float32(math.Sin(float64(angle*(1-t)))) * s
		s2 := float32(math.Sin(float64(angle*t))) * s
		this.W = w1*s1 + w2*s2
		this.X = x1*s1 + x2*s2
		this.Y = y1*s1 + y2*s2
		this.Z = z1*s1 + z2*s2
		...
		this.Y = y1 + t*(y2-y1)
		this.Z = z1 + t*(z2-z1)
		len := float32(1.0 / math.Sqrt(float64(this.W*this.W+this.X*this.X+this.Y*this.Y+this.Z*this.Z)))
		this.W *= len
		this.X *= len
		this.Y *= len
		this.Z *= len
